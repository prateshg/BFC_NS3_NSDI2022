...
UdpClientHelper::UdpClientHelper (Ipv4Address address, uint16_t port, uint16_t pg, uint32_t q, uint32_t dr)
{
  m_factory.SetTypeId (UdpClient::GetTypeId ());
  SetAttribute ("RemoteAddress", AddressValue (Address(address)));
  SetAttribute ("RemotePort", UintegerValue (port));
  SetAttribute ("PriorityGroup", UintegerValue (pg));
  SetAttribute ("Queues", UintegerValue (q));
  SetAttribute ("DataRate", UintegerValue (dr));
}
