...
namespace ns3 {
	...
	{
	public:
		static const unsigned qCnt = 36;	// Number of queues/priorities used
		static const unsigned pCnt = 64;	// Number of ports used
		static const uint32_t hqCnt = pCnt*qCnt*200;
		Entry central_entry_array[hqCnt];
		...
		//yibo
        void SetNode(Ptr<Node> node, uint32_t sid);
        void CheckAndSendPause();
		...
		void UpdateIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize, uint32_t oport);
		void UpdateEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize);
		void RemoveFromIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize, uint32_t oport);
		...
		void GetBpPauseClasses(uint32_t port, uint32_t qIndex, bool pClasses[]);
		bool GetBpResumeClasses(uint32_t port, uint32_t qIndex);
		...
		int m_usedIngressBytes[pCnt][pCnt][qCnt];
		uint32_t m_node_id;

		void SetStats(bool s);
		bool enable_buffer_stats;
